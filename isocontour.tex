...
However, if the error caused by discarding a packet is of subvoxel resolution, such a metric does
not capture the importance of that packet, causing~\Cref{alg:greedy} to produce a suboptimal \siop.
Therefore, we add the relative difference in surface areas ($|A_1-A_2|/|A_1|$) to the error term.
This additional term is most of the time between $[0, 1]$, and is meant to capture the subvoxel
...
outperforms \sisg in this case, most likely because local signatures coming from regions where the
