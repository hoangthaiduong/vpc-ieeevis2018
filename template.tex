...
%% Paper title.
\title{A Study of the Trade-off Between Reducing Precision and Reducing Resolution for Data Analysis and Visualization}
...
\abstract{There currently exist two dominant strategies to reduce data sizes in analysis and
  visualization: reducing the \emph{precision} of the data, e.g., through quantization, or reducing
  its \emph{resolution}, e.g., by subsampling. Both have advantages and disadvantages and both face
  ...
  a principled manner. We represent data reduction schemes as progressive \emph{streams} of bits and
  study how various bit orderings such as by resolution, by precision, etc., impact the resulting
  approximation error across a variety of data sets as well as analysis tasks. Furthermore, we
  compute streams that are optimized for different tasks to serve as lower bounds on the achievable
  error. Scientific data management systems can use the results presented in this paper as guidance
  on how to store and stream data to make efficient use of the limited storage and bandwidth in
  practice.\vspace{-0.5em}
...
\keywords{data compression, bit ordering, multi-resolution, data analysis}
...
\teaser{
  ...
  \vspace{-1.5em}
  \caption{Visualization of the \emph{diffusivity} field at 0.2 bits per sample (bps) and its Laplacian field at 1.5 bps, using two of the bit streams studied in the paper. Compared to the \emph{by bit plane} stream, the \emph{by wavelet norm} stream produces a better reconstruction of the original function (left, compare white features), and a slightly worse, if not comparable, reconstruction of the Laplacian field (right).}
  \label{fig:teaser}
}
...
\appendix{
  \label{apd:norm-basis_funcs}
  \section{L2 Norms of CDF5/3 wavelet basis functions}
  The $\swav$ stream requires the norms of CDF5/3 wavelet basis functions, which the
  following pseudocode computes (in 1D).
...
for (l = 0; l < n+1; l = l+1) {
  ...
  \end{Verbatim}
  The \texttt{upsample} function adds a 0 between every two adjacent samples in its argument. The
\texttt{convolve} function implements the convolution operation in 1D. When the loop ends, \texttt{sca\_norms[n]}
stores the the norm of the basis functions in the coarsest subband (they all share the same norm),
while \texttt{wav\_norms[n-1], wav\_norms[n-2]}, \dots store the norms of basis functions in
subsequent subbands, from coarse to fine. Since we use tensor product wavelets, the norms of basis
functions in higher dimensions are simply products of the 1D norms.

%% if specified like this the section will be committed in review mode
\acknowledgments{We thank the reviewers for their valueable comments. Some of the data sets are
  kindly shared with us by Benjamin Isaac (\emph{boiler}) and Kristoffer Matheson and
  Ashley Spear (\emph{foam} -- collected through work supported by NSF grant
  DMREF-1629660). This work was performed under the auspices of the U.S. Department of Energy by
  Lawrence Livermore National Laboratory under Contract DE-AC52-07NA27344 and was supported by the
  LLNL-LDRD Program under Project No. 17-SI-004. This work is supported in part by NSF: CGV:
  Award:1314896,  NSF:IIP Award :1602127 NSF:ACI:award 1649923, DOE/SciDAC DESC0007446, CCMSC
  DE-NA0002375, and PIPER: ER26142 DE-SC0010498. This material is based upon work supported by the
  Department of Energy, National Nuclear Security Administration, under Award Number(s)
  DE-NA0002375. This work was also supported by the Department of Energy Office of Science, Office
  of Advanced Scientific Computing Research under the guidance of Dr. Lucy Nowell.}
