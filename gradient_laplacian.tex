...
we are unaware of any widely accepted solutions to this problem.~\Cref{sec:gradient}
...
We further investigate this difference by extracting a 1D line from \emph{plasma} data set and reconstructing
the function using \sbit and \swav at 0.6 bps (\Cref{fig:bit-plane-vs-wavelet-norm-gradient}). Because the
...
\caption{Laplacian error comparison among streams. The plots are truncated so as to better highlight
differences without discarding important information. In all cases, in terms of error,
$\slop < \slsg < \sbit < \swav < \smag < \slvl$}.
...
\emph{pressure}, at 1.5 bps.  In terms of image quality, $\slsg > \sbit > \swav
