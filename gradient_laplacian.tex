...
The numbers in brackets are the ranges of gradient magnitudes. The trend in error, in
all cases, is $\sgop < \sgsg \approx \sbit \approx \swav < \smag < \slvl$.}
...
Laplacian fields. In all cases, in terms of error, $\slop <\slsg < \sbit < \swav < \smag < \slvl$.}
...
f)=\text{RMSE}(\Delta f',\Delta f)$. As usual, we use~\Cref{alg:greedy} to compute a
