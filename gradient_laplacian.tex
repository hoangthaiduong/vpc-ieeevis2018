...
Computation of derivative-based quantities is important in data analysis. Since simulation data can
rarely be captured by closed-form formulae, we resort to finite differences for derivative
computation. In this section, we use 32 bits for quantization to ensure enough precision for finite
differences, as compared to 16 bits for other experiments. We always perform finite differences on
the finest resolution to avoid computing distances between quantities defined on grids of different
resolutions.
...
% In \Cref{fig:laplacian-renderings}, we render the reconstructed Laplacian fields for the
