...
In this section, we study the gamut of fundamental analysis tasks, namely function reconstruction
...
evaluating the performance of different streams on the task. We use~\Cref{alg:greedy} to
...
\begin{figure}[t]
  \centering
	\subcaptionbox{\emph{boiler}}{ {\includegraphics[width=0.48\linewidth]{rmse/rmse-optimized-boiler}}}
		...
		Lower RMSE is better. The streams are truncated to highlight the differences, without omitting
...
\begin{figure}[t]
	...
	\caption{Volume renderings of a $64^3$ region of \emph{plasma} at 0.1 bps. $s_{lvl}$ captures the background (purple-blue) well, whereas $s_{bit}$ captures the fine details better. $s_{wav}$
...
streams are shown in~\Cref{fig:rmse-optimized}. The main trend in all cases is $s_{opt} > s_{sig} >
...
much smaller. They contain the majority of the leading zero bits, whose removal
benefits $s_{bit}$ the most, as it touches fine-resolution bits the earliest. $s_{mag}$
underperforms for the same reason that $s_{lvl}$ does, but to a lesser extent, due to it being
better adaptive to the data. $s_{wav}$ outperforms both $s_{lvl}$ and $s_{bit}$, because it follows the
optimal (data-independent) bit ordering in $S_{L,B}$\pavol{what is this?} in the $L_2$ norm, which is also the norm that
RMSE is based on. $s_{opt}$ unsurprisingly outperforms the rest, being the most data-adaptive.
...
a thin ``surface''. For such data, wavelet coefficients along one dimension are often larger
compared to those along other dimensions, which $s_{opt}$, and hence $s_{sig}$, can take
