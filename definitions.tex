...
because \hbadd{the amount of change in one step of multiresolution
...
\Cref{sec:data_dep_streams} presents a greedy algorithm that constructs a reference
...
dimensions\pavol{could be clearer}. A subsequent transform pass \hbdel{typically} recurses on the first
...
0$ referring to the coarsest subband and $L$ \hbadd{denoting the number of
...
less significant.}\pavol{really?} \hbdel{We use $b$, which starts from 0, to index the bit
...
Additionally, we define a third stream that combines these two dimensions and refer
to it as \emph{by wavelet norm}, \swav.  This stream orders packets using
weights $\wwav(p)=2^{B-1-b(p)}\times \norm{\omega_{l(p)}}^2$\pavol{by bit plane does not have
the -1 term, is this calculation correct?}. The first term
...
Another common data reduction technique, especially for transform-based data\pavol{what sort of data is it?},
is one in which truncation happens leaving out coefficients of smallest
magnitudes.\pavol{ambiguous sentence, is the truncation removing whole coefficients?} We model this scheme with a stream called \emph{by magnitude},
...
modify this greedy algorithm to build the stream backwards.\pavol{check if it is true} In other words, we
...
complexity is $\mathcal{O}(n^2)$\pavol{technically, it is $n^3$}, with $n$ being the total number of packets in
...
ordering of packets. To concisely represent and characterize the
...
plane of the second subband ($\sgn(1,2)=5$). Thus, a stream's signature shows
