...
\subsection{Decomposition of data into packets}
...
way. From its definition, a packet can always be associated with a subband $l$ ($0\leq l\leq
\bar{l}$) and a bit plane $b$ ($0\leq b\leq \bar{b}$).
...
assumed to have received a subset of packets from the server, so it can reconstruct the original
function from these packets (using the inverse wavelet transform). Therefore, to compare a set of
streams, we can extract the same number of packets to reconstruct the original data from each, and
perform analysis tasks on each of these (approximate) reconstructions. A stream is better suited for
an anlysis task if it produces results that are closer to reference results computed from the
original data.
