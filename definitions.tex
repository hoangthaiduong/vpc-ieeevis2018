...
bit plane $b_p$ and subband $l_p$, to the whole function $f$. This ordering based on norms of
...
full (instead of empty) set of packets, our error measurement better captures the importance of
packets.
...
observation) retaining the same quality for \sopt. \Cref{alg:greedy} outlines the approach described
above.
...
In particular, $\sgn_{l,b}$, i.e., the $(l,b)$ element of $\sgn$, is an integer in the range $[0,
(1+B)\times L)$, and indicates, on average, the position at which packets in $P_{l,b}$ appear in the
given stream. For example, the signature $\sgn=\bigl[ \begin{smallmatrix}0 & 1 & 4\\
...
the end of \ssig all the packets in $P_{l,b}$. An \ssig captures the behavior (in the
$\Lspace_{L,B}$ space) of the stream it derives from, but it is stripped from any spatial
adaptivity. Hence, when \ssig is derived from an \sopt, it can serve as a bridge when comparing the
