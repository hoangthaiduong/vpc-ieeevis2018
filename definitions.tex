...
tradeoffs among different data reduction schemes, it is important to perform
fair and consistent comparisons.  In this section, we develop such a
consistent methodology by proposing to model different data reduction schemes
as \emph{streams} of uniformly sized data \emph{packets}, where the original
data contains all of the packets, and any reduction step removes a set of
packets (comparable amounts of data).}

\hbadd{These data streams are transmitted using a client-server model. At any
point, the client is assumed to have received a subset of packets from the
server, which can be used to reconstruct an approximation to the original
data. Therefore, to compare different streams, we can reconstruct the original
data using the \emoh{same number} of packets from each stream, and perform
desired tasks on each of the (approximate) reconstructions. A stream is
considered better suited for a given task if it produces results that are
closer to the ``ground truth'', i.e., the reference results computed from the
original data. \Cref{fig:pipeline} gives a schematic view of our data
streaming model.}

\hbadd{Although both the server and the client in our model can be on the same
physical machine, only the server has full knowledge of the data. Thus, when
the client receives a packet, it might not know where that packet should be
deposited. A common solution is to have both the client and the server agree
beforehand on a static ordering of packets, independent of the data. We use
the term \emph{data-independent streams} to refer to streams using such
solutions. In contrast, for \emph{data-dependent streams}, an additional
mechanism is needed to inform the client about the position of incoming
packets.  In this paper, we consider both these types of streams, as well as
...
happen immediately after wavelet transform. \todo{TODO: revise this
...
%{0.01\textwidth} l{0.04\textwidth} c{0.04\textwidth} c{0.04\textwidth}}
\hline
...
%It is important to compare the data reduction techniques fairly in order to
%systematically study the resolution-versus-precision tradeoffs among them.
...
%multiresolution scheme uses the same amount of data as does a quantization
%scheme, because the amount of change in one step of multiresolution
%simplification may be different from removing one bit in the quantization of
%every data sample. To avoid such mismatch of data increment/decrement units,
%we model each reduction scheme as a stream of equally sized \emph{packets}.
%We define a packet as the smallest unit of data increment/decrement in our
%framework, where the packet consists of a relatively small number of bits
%from a data set. We associate a packet with a resolution level and a
%precision level (i.e., bit plane), so that each packet can improve either the
%data's resolution or its precision. In this framework, different
%data-reduction schemes become different orderings of packets, and we call the
%sequence of packets \emph{stream}. The property of each stream having the
...
\hbadd{Although one way to compare different data reduction strategies is}
\hbdel{It is important to compare the data reduction techniques fairly in
order to systematically study the resolution-versus-precision tradeoffs among
them. One popular approach is} to restrict the techniques to the same data
size and compare data quality, it \hbdel{is, however, difficult to enforce
that a multiresolution scheme uses the same amount of data as does a
quantization scheme, because the amount of change in one step of
multiresolution simplification may be different from removing one bit in the
quantization of every data sample. } \hbadd{becomes difficult to enforce
consistency. For example, the amount of change (in data) in one step of
multiresolution simplification may be different from removing one bit in the
quantization of every data sample.} To avoid such mismatch of \hbadd{units of}
data increment/decrement\hbdel{ units}, we model each reduction scheme as a
stream of equally sized \emph{packets}\hbadd{, which, we define as}\hbdel{. We
define packet} as the smallest units of data \hbadd{transfer}
\hbdel{increment/decrement} in our framework. \hbdel{, where the} \hbadd{A}
packet consists of a relatively small number of bits from a data set. We
associate a packet with a resolution level \hbadd{or} \hbdel{and} a precision
level (i.e., bit plane), \hbadd{such} \hbdel{so} that each packet can improve
either the \hbdel{data's} resolution or \hbdel{its} \habdd{the} precision
\hbadd{of data}.  In this framework, different data-reduction schemes become
different orderings of packets, \hbdel{and we call the sequence of packets}
\hbadd{called as} \emph{stream}.  \hbadd{Restricting two (or more) data
streams to the same} \hbdel{The property of each stream having the same total}
number of packets allows us to perform fair \hbadd{and consistent}
comparisons.

\para{Packets for Different Resolution Levels.} \hbdel{Each new packet adds to
the data's resolution or precision.To introduce the concept of resolution, we
use the popular CDF5/3 discrete wavelet transform~\cite{cdf-wavelets}.}
Although there exist \hbadd{several ways to define the notion of} \hbdel{other
transforms that introduce the concept of} resolution/scale/frequency, we
choose wavelet transform \hbadd{due to the compact support of their basis
functions,} \hbdel{because wavelet coefficients are spatially localized,}
which enables spatial adaptivity (i.e., finer resolution in regions that
contain sharp features, at the expense of coarser resolution elsewhere).
\hbadd{In particular, we choose the CDF5/3 discrete wavelet
transform~\cite{cdf-wavelets}} \hbdel{We choose the CDF5/3 wavelet} for its
balance between simplicity and effectiveness at decorrelating the input signal
in practice~\cite{jpeg2000}.

\hbadd{A} \hbdel{The} multidimensional wavelet transform can be performed in
multiple passes, which \hbadd{partitions} \hbdel{eventually results in} the
original domain \hbdel{being partitioned} into \hbdel{several}
\emph{subbands}, each of which can be thought of as a resolution level. One
transform pass \hbadd{(in 3D)} creates eight subbands \hbdel{in 3D}, of which
the first is a smoothed, downsampled version of the original data, and the
\hbadd{remaining} \hbdel{next seven} add fine details in each subset of the
dimensions (see \hbdel{a visualization of 2D subbands in~}
\Cref{fig:pipeline}). A subsequent transform pass recurses only on the first
subband \hbadd{creating the next resolution of subbands}. We use $l$ $(0 \leq
...
\hbdel{(in 3D)}, corresponding to three transform passes \hbadd{in 3D}.

\para{Packets for Different Precision Levels.} For creating packets
...
g\times g]$ grid of adjacent coefficients on the same subband. \hbdel{$g$ is
always $2$ for the experiments in this paper.} We let $g$ be a constant
\hbadd{($=2$ in this paper)}, so that finer resolution subbands contain more
...
%\para{Data Streams.} A data reduction scheme can be modeled as stream of
%packets, where the original data contains all packets, and any reduction step
%removes certain packets based on some criteria (details in
%\Cref{sec:static-dynamic-streams}).  These data streams are provided using a
%client-server model. At any point, the client is assumed to have received a
%subset of packets from the server that it can use to reconstruct the original
%function (using the inverse wavelet transform). Therefore, to compare
%different streams, we can reconstruct the original data using the same number
%of packets from each stream, and perform analysis tasks on each of the
%(approximate) reconstructions. A stream is better suited for a given analysis
%task if it produces results that are closer to the ``ground truth'', i.e.,
%the reference results computed from the original data. \Cref{fig:pipeline}
...
Additionally, we define a third
...
bit plane $b_p$ and subband $l_p$, to the whole function $f$. \hbdel{To our knowledge, a}
\hbadd{A} data-reduction
...
In \hbadd{principle}\hbdel{theory}, data-dependent streams are better than data-independent streams because they can
...
Each analysis task \hbadd{may} \hbdel{potentially} require\hbdel{s} a fundamentally different stream for optimal results.
...
stream at a later point. In order to avoid local minima, we \hbadd{make} \hbdel{consider} a modification to this greedy
algorithm\hbadd{, and} \hbdel{to} build the stream backwards. We start with a ``lossless'' $\tilde{f}$ (i.e.,
...
complexity comes from the complexity of the 2-level nested loop \hbdel{($\mathcal{O}(n^2)$)}, multiplied by
that of the wavelet transform and error computation \hbdel{($\mathcal{O}(n)$)}. Considering a small, e.g.,
...
\hb{I think you changed $\lambda$ to $\Sigma$. i think they should 
be the same. the space you're defining here is the same as signature.
remember, $l,b$ are integers, in which case the 2D space you talk about is 
nothing but a $L\times B$ matrix}

\hb{we should define signature as the space.. not as matrix}
