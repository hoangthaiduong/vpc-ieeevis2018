...
position of incoming packets. \todo{PL: In practice, does not the client inform the server where it wants more data?  How can the server know where more data is needed?} In this paper, we consider both types of streams, as well as
...
the output is a stream of \emph{packets}\todo{PL:So a packet is a set of bits from wavelet coefficients?  We need to explain what a packet is.}. Different data reduction schemes generate different streams.  The
...
after wavelet transform. \todo{TODO: revise this figure}
...
precision of data. \todo{PL: Does a packet the improves resolution add the first bits of those 
samples/coefficients?  Need to make it clear what we mean by adding resolution, especially in the 
context of wavelets.} 
...
%
\hbadd{Since 
%the bits in the quantized values are initialized with zero, and because the 
disambiguation between small negative and large positive values in the standard two's complement 
representation requires knowing the sign bit,
%standard two's complement assigns a small negative value (e.g., $-1$) a bit in high bit plane (e.g., $2^{B-1}$), 
%which can be distinguished from a large positive value only with the help of the sign bit, 
we instead use a negabinary representation, 
in which there is no single dedicated sign bit, and the bit ordering for an unsigned number 
is roughly proportional to the magnitude of the signed value. In particular, 
integers are represented in base $-2$, i.e., $\sum_{i=0}^{B}{c_i(-2)^i}$ with $c_i\in \{0,1\}$. 
%Negabinary representation has no single dedicated sign bit, and the bit ordering for an unsigned number 
%is roughly proportional to the magnitude of the signed value. 
Therefore, the negabinary form ensures that small 
coefficients have many leading zero-bits.}
%
%\hbdel{To avoid special treatment of the sign bit, we convert quantized coefficients from two's complement
%form to a negabinary form, in which integers are represented in base $-2$, i.e.,
%$\sum_{i=0}^{B}{c_i(-2)^i}$ with $c_i\in \{0,1\}$.}
This transformation increases the number of bit
planes by one \hbadd{(note the summation from $0$ to $B$ in the above expression)}, 
to compensate for the absence of the sign bit. In our convention, the higher indexed
bit planes are less significant.
\todo{PL: They’d better not be!  If i is the index and $(-2)^i$ 
is the place value of a bit, then higher i means *more* significant.}
...
and packets that provide finer (but more local) details. Every packet \hbadd{(one byte)} 
...
\todo{PL: A packet is a block (?), so row-major order of *blocks* of coefficients.} and
then bit plane order (from 0 to $B$) within each coefficient. 
\todo{PL: Do we get all bits for one coefficient, or  one bit plane at a time for the block of coefficients?} 
...
\todo{PL: Bit planes are ordered from 0 to B, so $2^{(B-b(p))}$.}
...
basis function 
%
\todo{I think you want to point out that $||$ refers to the L2 norm of the basis function. 
When wavelet transforms are based on lifting, the norm is usually not one, but increases with level.
...
magnitudes\hbadd{, since coefficient magnitude is only weakly related to error, as the error 
also depends on the wavelet basis function norm~\cite{weiss}}.
...
\todo{PL: By this you mean that you get the position information for free?  I.e. this info is not part of the stream itself?  Clarify this important distinction since the previous sentence talks about encoding this information.}
can
...
respect to \todo{what? bit rate? data size?}.

\todo{see PL's original comments in this paragraph}
...
Given a stream, we define its signature $\sgn = \sgn^{L,B}$ as an $L \times (1+B)$ matrix, where each $(l,b)$
...
\todo{Why?  Does that result in a different signature?  If so, why is this preferred?} 
It is only
...
17$ image, corresponding to $10$ subbands \hbadd{(in 2D)} and $17$ bit planes. Each $(l,b)$ ``cell'' contains a
unique value from $0$ to $169$, indicating its ``priority'' in the stream, and 
\hbadd{mapped to a white--blue color scale} 
\hbdel{which is mapped through a linear blue color map ($0$ is mapped to the brightest blue)}. It is clear 
\todo{PL: No, it is not clear, because you haven’t told us what the ordering of sub-bands is.}
