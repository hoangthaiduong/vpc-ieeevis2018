...
%A fair comparison of data reduction techniques is challenging. 
...
because \hbadd{the amount of data change in one step of multi-resolution
...
The \hbdel{typical} multi-dimensional wavelet transform can be performed in
...
\hbadd{$L=22$} \hbdel{$L=21$} (in 3D), corresponding to three passes of the
wavelet transform. \hb{changed the defn of $L$ to make it consistent with $B$
...
``server'' which serves data and a ``client'' which receives data from the
...
While both the server and the client in our model can be on the same physical
...
data, packets follow the row-major order of coefficients, then bit plane order
...
bit plane, packets follow the subband order (from 0 to $L$), then row-major
...
captures the contribution of a bit on bit plane $b(p)$, while the second term
...
is one in which truncation happens by leaving out coefficients of smallest
...
subband index, then by bit plane. Unlike \slvl and \sbit, \smag
