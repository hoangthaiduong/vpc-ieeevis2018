...
\peter{Confusing---IDX is a multires technique that uses subsampling, which
introduces aliasing.  *Filtering* when already doing multires is what
reduces aliasing.}
Especially popular are approaches that create
multiple levels of detail
\peter{levels of detail, not level of details}
...
\peter{Discuss Gaussian and Laplacian pyramids, which unlike wavelets use
redundant storage~\cite{laplacian-pyramid}.}
...
The wavelet transform constructs a hierarchy of resolution levels via
...
\peter{Maybe mention QCCPACK (http://qccpack.sourceforge.net), which
supports (patented) SPIHT coding.}
...
\peter{Discuss scalar~\cite{sqe} vs. vector quantization~\cite{hvq}.
SZ performs residual scalar quantization~\cite{sz}. \cite{fpzip}
truncates floats, which can be seen as nonuniform scalar quantization.}
...
\peter{Should the following go in a section on ``hybrid'' precision and
...
\peter{SPECK is just a simpler version of SPIHT.  We can cite it, but
needs no furhter discussion.}



Volume rendering stuff
Compression domain volume rendering~\cite{compression_domain2003}
\peter{This is \cite{hvq} above.}
...
\peter{We might want to cite~\cite{codar} and~\cite{li} for surveys
on data reduction.}
