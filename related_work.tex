...
For surveys of data reduction techniques in general, we refer the readers to the work of
Rodr\'{\i}guez \etal~\cite{state-of-the-art-compressed-volume} and Li \etal~\cite{li2018}.

%\paragraph{\textbf{Tree-based multiresolution hierarchies}}
...
from higher-resolution ones through filtering and subsampling\pavol{both? I think they are exclusive}. Examples include Gaussian and
Laplacian pyramids~\cite{laplacian-pyramid},
mipmaps~\cite{multires_octree1999,interactive-exploration-ct-scans}. Often, the data is stored
...
increase storage requirements, making them unsuitable for very large data.\pavol{They do, but usually
it is small fraction of the data, for example mipmap in 3D is 15\% overhead}
...
subsampling can introduce heavy aliasing artifacts.\pavol{is there some paper that explores this aliasing?}
...
COVRA~\cite{covra2012} constructs an octrees of bricks\pavol{why plural? is the octree computed per brick
or octree having bricks as leaves?}, which are further subdivided blocks\pavol{bricks are subdivided into blocks?}.
...
data and can achieve very high ratios of quality over data size\pavol{what does it mean? compression ratio?}. However, the transform step is
highly data-dependent, thus relatively costly.\pavol{why costly? do we need to transmit something?}
...
wavelet coefficients and basis functions. One disadvantage of the wavelet transform is the random access cost
which is not constant time. However, there has been work to develop acceleration structures to speed up local
queries~\cite{weiss}.
...
compression) are used in a wide variety of systems~(\cite{multires_toolkit2003,vapor2007,woodring2011}), and applications (volume rendering with
...
The \zfp compression scheme~\cite{zfp2014} also codes transform coefficients by bit plane\pavol{encodes transformed coefficients...}, in order
of decreasing significance. \zfp partitions the domain---a structured grid---into small (e.g., $4
\times 4 \times 4$\pavol{if zfp supports only this block size, then remove e.g.,}) independent blocks and thus allows for localized decompression. Moreover, \zfp
supports fixed-rate compression that facilitates random access to the data. Its fast transform and
...
quantization. Another work that uses scalar quantization is~\cite{sqe}\pavol{expand the sentence}. Similar to FPZIP, the
...
bytes, and each of the other six parts contains one byte for additional precision. MLOC includes
a multiresolution scheme based on Hillbert curves, but this scheme (based on resolution)
...
between. Outside image compression, JPEG2000 has found use in compression of scientific data.
...
bit rate. In general, although there exists mechanisms that simultaneously leverage both
axes of data reduction, especially for the purpose of scientific data analysis; however these have not been
well-studied.
...
focus on order-of-accuracy\pavol{is this $C^n$ stuff?} and convergence analysis, where errors are introduced mostly through
